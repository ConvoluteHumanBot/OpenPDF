...
public class CMapAwareDocumentFont extends DocumentFont {
     ...
     * values into unicode equivalent
     */
    ...
    /**
     * For all widths of all glyphs, compute the average width in normalized 1000 point units. This is used to give some
     * meaningful width in cases where we need an average font width (such as if the width of a space isn't specified by
     * a given font)
     *
    ...
    /**
     * @see org.openpdf.text.pdf.DocumentFont#getWidth(int)
    ...
    /**
     * Decodes a string of bytes (encoded in the font's encoding) into a unicode string. This will use the ToUnicode map
     * of the font, if available, otherwise it uses the font's encoding
     *
     ...
     * encoding for the font, this will fail. Required since we need to process the characters of strings, and we can't
     * determine the character boundaries in advance, especially because of Identity-H encoded fonts which have two-byte
     * character indexes.
     * <p>
     * PdfString is used to hold character code points, even though the bytes may not map 1-1. It's not possible to
     * change the encoding once a string is in place.
     *
     * @param chars the Characters that need to be decoded
     * @return the unicode String that results from decoding
     * @since 2.1.
     */
    ...
    /**
     * Decode  single character whose value represents a code point in this font. Will fail if the characters do not
     * have values that correspond to valid code points for the font.
     *
